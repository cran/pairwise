#' @title ML Person Parameter dichotomous 1PL
#' @export ppML.dicho
#' @description Calculation of the person parameters for dichotomous items, given the item parameters and and the datamatrix (argument \code{daten}) containing the person respose vectors (rows), according the 1PL Rasch Model using an ML approach.
...
#' @return An object of class "ppMLdicho" or "list" (depending on \code{techout}) containing the person parameters.
#' @exportClass ppMLdicho
#' @examples ############
...
ppML.dicho<-function(daten,SIGMA,incidenz=NULL,na_treat=0,limit=0.00001,iter=20,tecout=FALSE){ 
...
if (length(incidenz)==0){
object<-(apply(daten,1,thetaML.dicho, sigma=SIGMA, na_treat=na_treat, limit=limit,iter=iter))
...
class(res) <- c("ppMLdicho","data.frame")
}
...
if (length(incidenz)!=0){
 ...
 object <- lapply(daten_booklet_list, function(x){apply(x$daten,1,thetaML.dicho, sigma=x$sigma, na_treat=na_treat, limit=limit,iter=iter) }  )
